...
#Les plotejem per veure com triar quants components principals
...
props <- vars / sum(vars)
acumul <- cumsum(props)
plot(acumul[1:200], type = "l", xlab = "N? Components Principals", ylab = "% Explicat", log = "x", panel.first = grid())
plot(acumul[1:200], type = "l", xlab = "N? Components Principals", ylab = "% Explicat", panel.first = grid())
#podem observar que:
...
#Fem una primera aproximaci? sense tocar els hiper-parametres
...
model.CV.SVMRBF
#Tal com es pot veure ens els resultat del CV, els millors valors per els hiper-parametres gamma i cost s?n,  0.015625 i 8 respectivament.
